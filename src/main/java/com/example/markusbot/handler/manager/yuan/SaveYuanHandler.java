...
public class SaveYuanHandler implements Handler {
    ...
    public Object handle(Message message) {
        if (!NumberUtils.isCreatable(message.getText()) || Double.parseDouble(message.getText()) <= 0) {
            return SendMessage.builder()
                    ...
                    .text("Неверный ввод, введите положительное число для того чтобы поменять курс ЮАНЯ!")
                    .build();
        }
        settingService.updateYuan(Double.valueOf(message.getText()));
